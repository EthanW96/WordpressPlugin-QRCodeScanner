<?php
// City QR Code Report functionality
class QRCodeTracker_CityReport {
    private $main_table;
    private $log_table;
    private $tracker;
    private $teams;
    ...
    /**
     * Display the city QR code report page
     */
    public function display_city_report_page() {
        global $wpdb;
        ...
        if (empty($city)) {
            echo '<div class="wrap"><h1>City QR Code Report</h1>';
            echo '<div class="error"><p>No city specified.</p></div>';
        ...
        $this->display_city_details($city);

        // Filters Section
        $this->display_filters($city, $date_from, $date_to);

        $scan_where_clause = $this->build_scan_only_where_clause($where_clause);

        // Summary Statistics
        $this->display_summary_stats($scan_where_clause, $where_params, $city);

        // Charts Section
        $this->display_charts($scan_where_clause, $where_params, $city);

        // QR Codes in this City
        $this->display_city_qr_codes($where_clause, $where_params, $city);

        // Detailed Scan Logs
        $this->display_scan_logs($where_clause, $where_params, $city);
     ...
     */
    private function display_city_details($city) {
        global $wpdb;
        
        // Get city statistics with team access restrictions
        list($team_restriction, $team_params) = $this->build_team_access_restrictions();
        $city_stats = $wpdb->get_row($wpdb->prepare(
            "SELECT 
            ...
            WHERE t.city = %s{$team_restriction}",
            array_merge([$city], $team_params)
        ));

        echo '<div style="background: #fff; border: 1px solid #ddd; padding: 20px; margin: 20px 0; border-radius: 4px;">';
        echo '<h2>City Details: ' . esc_html($city) . '</h2>';
        echo '<table class="form-table" style="margin: 0;">';
        echo '<tr><th style="width: 150px;">City:</th><td><strong>' . esc_html($city) . '</strong></td></tr>';
        echo '<tr><th>Total QR Codes:</th><td><strong>' . number_format($city_stats->total_qr_codes) . '</strong></td></tr>';
        echo '<tr><th>Total Scans:</th><td><strong>' . number_format($city_stats->total_scans) . '</strong></td></tr>';
        echo '<tr><th>Unique Postcodes:</th><td><strong>' . number_format($city_stats->unique_postcodes) . '</strong></td></tr>';
        echo '<tr><th>Unique Trees:</th><td><strong>' . number_format($city_stats->unique_trees) . '</strong></td></tr>';
        echo '<tr><th>Last Scanned:</th><td>' . esc_html($city_stats->last_scanned) . '</td></tr>';
        echo '</table>';
        echo '</div>';
    }
     ...
     * Display filters
     */
    private function display_filters($city, $date_from, $date_to) {
        echo '<div style="background: #f9f9f9; padding: 15px; margin: 20px 0; border: 1px solid #ddd; border-radius: 4px;">';
        echo '<h3>Filters</h3>';
        echo '<form method="get">';
        echo '<input type="hidden" name="page" value="qr-city-report">';
        echo '<input type="hidden" name="city" value="' . esc_attr($city) . '">';
     ...
     */
    private function display_summary_stats($where_clause, $where_params, $city) {
        global $wpdb;
     ...
     * Display charts
     */
    private function display_charts($where_clause, $where_params, $city) {
        // Include Chart.js
        echo '<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>';
        
        // Get time series data for this city
        ...
        echo 'plugins: {';
        echo 'title: { display: true, text: "Daily Scan Activity - ' . esc_js($city) . '" }';
        echo '},';
        echo 'scales: {';
        ...
        echo 'responsive: true,';
        echo 'plugins: {';
        echo 'title: { display: true, text: "Scan Distribution by Hour of Day - ' . esc_js($city) . '" }';
        echo '},';
        echo 'scales: {';
    ...
    /**
     * Display QR codes in this city
     */
    private function display_city_qr_codes($where_clause, $where_params, $city) {
        global $wpdb;
        
        // Get QR codes for this city
        $sql = "SELECT 
                    t.id,
                    t.postcode,
                    t.tree,
        ...
        $qr_codes = $wpdb->get_results($wpdb->prepare($sql, $city));

        echo '<div style="margin: 20px 0;">';
        echo '<h2>QR Codes in ' . esc_html($city) . '</h2>';
        
        if (!empty($qr_codes)) {
            echo '<div class="qr-table-responsive"><table class="widefat">';
            echo '<thead><tr><th>Postcode</th><th>Tree</th><th>Label</th><th>Reporting ID</th><th>Scans</th><th>Last Scanned</th><th>Actions</th></tr></thead>';
            echo '<tbody>';
            ...
            echo '<p>No QR codes found for this city.</p>';
        }
     ...
     * Display scan logs
     */
    private function display_scan_logs($where_clause, $where_params, $city) {
        global $wpdb;
        ...
        // Get scan logs
        $sql = "SELECT l.*, t.postcode, t.tree FROM {$this->log_table} l JOIN {$this->main_table} t ON l.tracker_id = t.id {$where_clause} ORDER BY l.scanned_at DESC LIMIT %d OFFSET %d";
        $query_params = array_merge($where_params, [$limit, $offset]);
        $logs = $wpdb->get_results($wpdb->prepare($sql, $query_params));

        echo '<div style="margin: 20px 0;">';
        echo '<h2>Recent Visit Logs for ' . esc_html($city) . '</h2>';
        
        // Export button
        $export_url = admin_url('admin.php?action=qr_tracker_export&export_type=city&city=' . urlencode($city));
        ...
        if (!empty($logs)) {
            ...
            echo '<div class="qr-table-responsive"><table class="widefat">';
            echo '<thead><tr><th>ID</th><th>Postcode</th><th>Tree</th><th>Source</th><th>Scanned At</th><th>Hour</th><th>Day of Week</th></tr></thead>';
            echo '<tbody>';
    ...
    /**
     * Get hour distribution data for this city
     */
